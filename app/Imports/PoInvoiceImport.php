...
class PoInvoiceImport implements ToModel, WithHeadingRow, SkipsOnFailure
{
    use SkipsFailures;

    public function model(array $row)
    {
        \Log::info('Processing po_invoice row: ' . json_encode($row, JSON_PRETTY_PRINT));
        ...
        if (empty($row['invoice_number']) && empty($row['Invoice Number']) && empty($row['invoice number']) && empty($row['Invoice No']) && empty($row['invoice no']) &&
            ...
            \Log::warning('Skipping po_invoice row: No valid data');
            return null;
        }

        \Log::info('Inserting into po_invoices: ' . json_encode($row, JSON_PRETTY_PRINT));
